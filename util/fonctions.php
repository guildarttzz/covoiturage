...
/**
 * code non encore écrit qui retournera le login 
 * s'il est présent en base ou une chaine vide,
...
function verifuser($login, $mdp) {
    foreach ( getLesUsers() as $logins) {
        if( $logins['user'] == $login && $logins['mdp'] == $mdp ){
            return $logins['id'];
        } else if ( $logins['user'] == $login ){
            break;
        }
    }
    return false;
...
/**
 * Récupère les utilisateurs du départ
 *
 * @return array
 * @since Version 2
 * @version 2
 */
function getUserLesOffresDepartEntreprise() {
    $tab = array();

    foreach (getLesOffresDepartEntreprise() as $offre) {
        if( !isset($tab[$offre['idchauffeur']]) ){
            $tab[$offre['idchauffeur']] = $offre;
        }
    }
